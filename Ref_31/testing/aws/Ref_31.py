print("This is Ref_31")

import json
from urllib.parse import unquote
import urllib3
urllib3.disable_warnings(urllib3.exceptions.InsecureRequestWarning)
import undetected_chromedriver as uc
import chromedriver_autoinstaller as chromedriver
chromedriver.install()
from bs4 import BeautifulSoup
import re
import certifi
import os
import sys
from datetime import datetime
import common_function
import pandas as pd
import requests
import warnings

warnings.filterwarnings("ignore")

import logging
urllib3_logger = logging.getLogger('urllib3')
urllib3_logger.setLevel(logging.CRITICAL)


headers = {
    'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/123.0.0.0 Safari/537.36',
}

duplicate_list = []
error_list = []
completed_list = []
attachment=None
url_id=None
current_date=None
current_time=None
Ref_value=None
ini_path=None

options = uc.ChromeOptions()
options.add_argument('--headless')
options.add_argument('--incognito')
options.add_argument('--disable-gpu')
options.add_argument('--disable-software-rasterizer')
options.add_argument('--disable-dev-shm-usage')
options.add_argument('--no-sandbox')
options.add_argument('--disable-infobars')
options.add_argument('--disable-extensions')
options.add_argument('--disable-popup-blocking')
options.add_argument('--user-agent=YOUR_USER_AGENT_STRING')
options.add_argument('--version_main=108')
driver = uc.Chrome(options=options)

headers2 = {
    "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/91.0.4472.124 Safari/537.36",
    "Accept": "text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,*/*;q=0.8",
    "Content-Type": "application/x-www-form-urlencoded"
}


try:
    print("Process started...")
    print("This may take sometime.Please wait..........")

    try:
        with open('Source_title.txt', 'r', encoding='utf-8') as file:
            url_list = file.read().split('\n')
    except Exception as error:
        Error_message = "Error in the \"urlDetails\" : " + str(error)
        print(Error_message)
        error_list.append(Error_message)
        common_function.attachment_for_email(url_id, duplicate_list, error_list, completed_list,
                                             len(completed_list),
                                             ini_path, attachment, current_date, current_time, Ref_value)

    for i, url_url_id in enumerate(url_list):

        try:
            url, url_id = url_url_id.split(',')
            print(f"Executing this {url}")
            current_datetime = datetime.now()
            current_date = str(current_datetime.date())
            current_time = current_datetime.strftime("%H:%M:%S")

            ini_path = os.path.join(os.getcwd(), "Info.ini")
            Download_Path, Email_Sent, Check_duplicate, user_id = common_function.read_ini_file(ini_path)

            data = []
            pdf_count = 1
            Ref_value = "31"
            url_value = url.split('/')[-2]
            current_out = common_function.return_current_outfolder(Download_Path, user_id, url_id)
            out_excel_file = common_function.output_excel_name(current_out)


            response = requests.get(url, headers=headers, timeout=50,allow_redirects=True)
            current_soup = BeautifulSoup(response.content, 'html.parser')
            current_issue  = current_soup.find("li",class_="rowShade").find("a")["href"]
            current_issue_link = "https://www.ingentaconnect.com"+current_issue

            response2 = requests.get(current_issue_link, headers=headers, timeout=50, allow_redirects=True)
            current_soup2 = BeautifulSoup(response2.content, 'html.parser')

            Articles = current_soup2.find_all("div",class_="ie5searchwrap")
            print(len(Articles))

            for article in Articles:
                Article_title = article.find("a").text
                Article_link = "https://www.ingentaconnect.com" + article.find("a")["href"]

                print(Article_title)
                print(Article_link)

                response3 = requests.get(Article_link, headers=headers, timeout=50, allow_redirects=True)
                current_soup3 = BeautifulSoup(response3.content, 'html.parser')

                supMetaData = current_soup3.find_all("div",class_="supMetaData")[2].text

                doi_pattern = r"DOI:\s*https://doi.org/(\S+)"
                volume_pattern = r"Volume\s(\d+)"
                date_pattern = r"(\d{1,2}\s\w+\s\d{4})"
                page_range_pattern = r"pp\.\s(\d+-\d+)"

                doi_match = re.search(doi_pattern, supMetaData)
                doi = doi_match.group(1) if doi_match else None

                volume_match = re.search(volume_pattern, supMetaData)
                volume = volume_match.group(1) if volume_match else None

                date_match = re.search(date_pattern, supMetaData)
                date = date_match.group(1) if date_match else None

                page_range_match = re.search(page_range_pattern, supMetaData)
                page_range = page_range_match.group(1) if page_range_match else None

                day, month, year = None, None, None
                if date:
                    day, month, year = date.split()

                print("DOI:", doi)
                print("Volume:", volume)
                print("Year:", year)
                print("Month:", month)
                print("Page Range:", page_range)

                a_tag = current_soup3.find("a",class_="fulltext pdf btn btn-general icbutton no-underline contain")
                print(a_tag)

                start_index = a_tag.find('data-popup="') + len('data-popup="')
                end_index = a_tag.find('"', start_index)
                data_popup = a_tag[start_index:end_index]

                base_url = "https://www.ingentaconnect.com"
                Pdf_link = base_url + unquote(data_popup)

                print("%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%55")
                print(Pdf_link)





            # year_issue = current_soup.find("div",class_="article-list-info listwrap").find('a').text
            #
            # match = re.match(r'(\d{4}), (\d+)\((\d+)\)', year_issue)
            #
            # year = match.group(1)
            # volume = match.group(2)
            # issue = match.group(3)
            #
            # print(year)
            # print(volume)
            # print(issue)
            #
            #
            # trial = 0
            # while trial<5:
            #     try:
            #         driver.get(current_url)
            #         time.sleep(20)
            #         page_content = driver.page_source
            #         current_soup = BeautifulSoup(page_content, 'html.parser')
            #         try_catch = current_soup.find("div", class_="article-list-info listwrap").find('a').text
            #         trial = 5
            #
            #     except:
            #         trial = trial+1
            #         print(f"Undeted trying to load {trial}")
            #         pass
            #
            # DOI_list = []
            # pdf_list = []
            # Articles = current_soup.findAll("div", class_="article-list-info listwrap fl")
            #
            # for article in Articles:
            #
            #     DOI = article.find('a',class_="mainColor ng-binding").text
            #     DOI_list.append(DOI)
            #
            # j = 1
            # articles_count_with_pdf = len(DOI_list)
            # for DOI in list(set(DOI_list)):
            #
            #     Article_link = "http://www.ysxb.ac.cn/en/article/doi/" + DOI
            #     response_2 = requests.get(Article_link, headers=headers, timeout=50, allow_redirects=True)
            #     current_soup_2 = BeautifulSoup(response_2.content, 'html.parser')
            #
            #     Article_title = current_soup_2.find('div',class_="article-title left-title").find("h1").text
            #     page_range_text = current_soup_2.find("span",class_="aIssue").find("a").text.strip()
            #
            #     page_range_pattern = r"\d{4}-\d{4}"
            #     try:
            #         page_range = re.search(page_range_pattern, page_range_text)[0]
            #     except:
            #         page_range = ''
            #
            #     print(page_range)
            #     print(Article_title)
            #     print(Article_link)
            #
            #     j = j + 1
            #     print("###############")
            #     check_value, tpa_id = common_function.check_duplicate(DOI, Article_title, url_id, volume, issue)
            #
            #     pdf_link_id = current_soup_2.find("div", class_="download-pdf fl")['data']
            #     Pdf_link = f"http://www.ysxb.ac.cn/en/data/article/check-article-pdf?id={pdf_link_id}"
            #     url_download = "http://www.ysxb.ac.cn/en/data/article/export-pdf"
            #     url_check = "http://www.ysxb.ac.cn/en/data/article/check-article-pdf"
            #     payload = {
            #         "id": pdf_link_id
            #     }
            #
            #     if Check_duplicate.lower() == "true" and check_value:
            #         message = f"{Article_link} - duplicate record with TPAID : {tpa_id}"
            #         duplicate_list.append(message)
            #         print("Duplicate Article :", Article_link)
            #     else:
            #         if Article_title not in pdf_list:
            #             print(pdf_link_id)
            #             response_check = requests.post(url_check, data=payload,headers=headers2)
            #             if response_check.status_code == 200:
            #                 json_response = response_check.json()
            #                 if json_response.get("result") == "success":
            #                     print("PDF file exists. Proceeding to download...")
            #                     time.sleep(5)
            #                     response_2 = requests.post(url_download, data=payload, headers=headers2)
            #                     if response_2.status_code == 200:
            #                         pdf_content = response_2.content
            #                         output_fimeName = os.path.join(current_out, f"{pdf_count}.pdf")
            #                         with open(output_fimeName, 'wb') as file:
            #                             file.write(pdf_content)
            #
            #                         data.append(
            #                             {"Title": Article_title, "DOI": DOI,
            #                              "Publisher Item Type": "",
            #                              "ItemID": "",
            #                              "Identifier": "",
            #                              "Volume": volume, "Issue": issue, "Supplement": "",
            #                              "Part": "",
            #                              "Special Issue": "", "Page Range": page_range, "Month": "",
            #                             "Day": "",
            #                              "Year": year,
            #                              "URL": Pdf_link,
            #                              "SOURCE File Name": f"{pdf_count}.pdf",
            #                              "user_id": user_id})
            #
            #                         df = pd.DataFrame(data)
            #                         df.to_excel(out_excel_file, index=False)
            #                         print(f"Downloaded the PDF file {pdf_count}")
            #                         pdf_count += 1
            #                         completed_list.append(Pdf_link)
            #                         with open('completed.txt', 'a', encoding='utf-8') as write_file:
            #                             write_file.write(Pdf_link + '\n')
            #                         pdf_list.append(Article_title)
            #                         print("###################################")
            #
            # try:
            #     common_function.sendCountAsPost(url_id, Ref_value, str(articles_count_with_pdf),
            #                                     str(len(completed_list)),
            #                                     str(len(duplicate_list)),
            #                                     str(len(error_list)))
            # except Exception as error:
            #     message = str(error)
            #     print("New update")
            #     error_list.append(message)

            if str(Email_Sent).lower() == "true":
                attachment_path = out_excel_file
                if os.path.isfile(attachment_path):
                    attachment = attachment_path
                else:
                    attachment = None
                common_function.attachment_for_email(url_id, duplicate_list, error_list, completed_list,
                                                     len(completed_list), ini_path, attachment, current_date,
                                                     current_time, Ref_value)

            sts_file_path = os.path.join(current_out, 'Completed.sts')
            with open(sts_file_path, 'w') as sts_file:
                pass

        except Exception as error:
            Error_message = f"Error in the site: {error}"
            print(Error_message, "\n")
            error_list.append(Error_message)
            common_function.attachment_for_email(url_id, duplicate_list, error_list, completed_list,
                                                 len(completed_list),
                                                 ini_path, attachment, current_date, current_time, Ref_value)

except:
    Error_message = "Error in the main process :"

    error_list.append(str(Error_message))
    log_file_path = os.path.join('log.txt')
    with open(log_file_path, 'w') as log_file:
        log_file.write(str(Error_message) + '\n')